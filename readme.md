...
Run `./setup_env.sh` (or `setup_env.ps1` on Windows) to create a Python virtual environment and install the dependencies listed in `requirements.txt`. The scripts set up `.ortho-venv/` in the project root. After installing the packages, start the development server with:

```bash
python run_server.py
```

Placeholder image utilities live under `src/`.
The basic frontend lives in `static/` and is served from the root URL.
...
* Since the majority of images are taken sequentially from a plane using some kind of automation, we can relatively safely predict the location of the next image in the sequence. The gap between two consecutive images is usually 3-5 km.
  ...
  * Above example shows 5th image of flight C-602. Flight numbering has no correlation with maps sheet number, which may also start like `C-5..`.
